    ...
    public class PrinterManager
    {
        ...
        public bool PrintCombSticker(Model m, String barCode)
        {
            try
            {
                ...
                CombStickerData = 
                    CombStickerData.Replace("{MRP}", m.MRP.ToString());

                CombStickerData = 
                    CombStickerData.Replace("MODELNAME", m.Name);

                CombStickerData = 
                    CombStickerData.Replace("STORAGECAPACITY", (Math.Round(m.StorageCapacity,1)).ToString());

                CombStickerData = 
                    CombStickerData.Replace("NETQUANTITY", (Math.Round(m.NetQuantity,1)).ToString());

                CombStickerData = 
                    CombStickerData.Replace("B163A>51401010001", m.Code + ">5" + barCode.Substring(4,6) + barCode.Substring(10,4));

                CombStickerData = 
                    CombStickerData.Replace("{W}X{D}X{H}", Convert.ToInt32(m.Width).ToString()
        ...
        public bool PrintCombSticker(Model m, String barCode,String template)
        {
            try
            {
                String CombStickerData = File.ReadAllText(template);
